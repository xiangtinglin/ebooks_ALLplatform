...
#             else:
#                 st.warning("請輸入正確的合約簡編/ISBN/單位名稱查詢")
#         else:
...
st.title("【總經銷】書單上架&分析")
st.markdown('<span style="color:red; font-weight:bold; font-size:22px;"> "Sorry～暫停服務！" </span>', unsafe_allow_html=True)

user_input = st.text_input("請輸入密碼(試試:6197):", type="password")
...
if user_input:
    if user_input in passwords_and_names:
        ...
        st.markdown(
            '<span style="color:red; font-weight:bold; font-size:22px;"> ｜STEP.1 _匯入檔案(目前檔案無資料庫化，因此需從你電腦匯入檔案) ↓</span>',
            unsafe_allow_html=True
        )

        with st.expander("請上傳Excel文件:(NEW)總經銷書單"):
            uploaded_file = st.file_uploader(
                "p.s.第一次載入大量數據需要數秒，之後查詢會很快^^，匯入後檔案預設存效4小時",
                type=["xlsx"]
            )
        ...
        def first_loading(file):
            return pd.read_excel(
                ...
                usecols=[0,2,3,8,17,19,23,25,29,31,36,38,42,44,48,50,54,56,60,62,66,68,72,74,78,80,84,86,90,92,96,98],
                engine="openpyxl"
            )

        if uploaded_file:
            data = first_loading(uploaded_file)
            ...
            st.write("> STEP.2 匯入檔案後，輸入條件查詢")
            keyword = st.text_input("*用合約'簡'編/ISBN/書名查詢").upper()

            if keyword:
                filter_contract = data["合約編號"].astype(str) == keyword
                filter_isbn = data["ISBN"].astype(str) == keyword
                filter_bookname = data["書名"].astype(str).str.contains(keyword, na=False)

                result = data[filter_contract | filter_isbn | filter_bookname]

                st.write(f"總共建檔數: {len(result)}")
                result.index = range(1, len(result) + 1)
                st.dataframe(result)
            else:
                st.warning("請輸入正確的合約簡編 / ISBN / 書名查詢")
        else:
            st.warning("請上傳 Excel 文件。")
    else:
        st.warning("密碼錯誤。")
