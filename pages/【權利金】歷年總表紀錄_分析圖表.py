        ...
        if uploaded_file:
            ...
            with st.expander("請選擇條件"):
                ...
                if view_option == "歷年加總":
                    ...
                    total_rank
                    # 列出的欄位
                    total_rank = grouped_data.groupby("合約簡編").agg({
                        '單位名稱': lambda x: r'\n'.join(x),  
                        '電子書內容收益': 'sum'}).reset_index()
                    
                    # 重新排序欄位順序
                    total_rank = result[['合約簡編', '單位名稱', '收益']]
                    total_rank
                    # total_rank = data.groupby(by=['合約簡編'])['電子書內容收益'].sum().reset_index().sort_values(by='電子書內容收益', ascending=False)
