...
if user_input:    
    ...
    if user_input in passwords_and_names:
        ...
        # 初始化 session_state 的 data 變數
        if 'data' not in st.session_state:
            st.session_state.data = None  # 預設為 None，後續會根據按鈕或上傳狀態進行賦值
        ...
        with st.expander("請上傳Excel文件:【權利金】歷年總表紀錄 "):
            ...
            if uploaded_file:
                st.session_state.data = fist_loading(uploaded_file)
                st.success("已成功加載上傳的檔案！")

        # 如果資料已經成功加載，進行後續資料處理
        if st.session_state.data is not None:
            # 使用 st.session_state.data
