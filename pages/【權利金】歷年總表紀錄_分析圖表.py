...
if user_input:
    ...
    if user_input in passwords_and_names:
        ...
        # 初始化 session_state 中的 data 和 data_source
        ...
        if st.button("使用內嵌測試檔案"):
            ...
            try:
                ...
                st.session_state.data_source = "測試檔案"
        ...
        with st.expander("請上傳Excel文件:【權利金】歷年總表紀錄 "):
            ...
            @st.cache_data(ttl=3600*4)  # 設定生存時間 (TTL) 為 4 小時
            ...
            # 如果上傳了檔案，則加載上傳的檔案並覆蓋現有資料
            if uploaded_file:
                st.session_state.data = load_uploaded_file(uploaded_file)
                st.session_state.data_source = "上傳檔案"
                st.success("已成功加載上傳的檔案！")

        # 如果資料已經成功加載，進行後續資料處理
        if st.session_state.data is not None:
            data = st.session_state.data.copy()  # 確保使用最新資料
            st.write(f"使用資料來源：{st.session_state.data_source}")
            ...
            INPUT_TEXT = st.text_input("輸入:合約簡編/ISBN/單位名稱查詢").upper()
            ...
            if INPUT_TEXT:
                ...
                with st.expander("STEP.3 _的條件會自動代入，拋出結果"):
                    ...
                    # 顯示篩選結果
                    st.markdown('<span style="color:blue; font-size:20px;"> 【一、以下是篩選後的權利金情形(可供下載)】 </span>', unsafe_allow_html=True)
                    result.index = range(1, len(result) + 1)
                    st.dataframe(result)

                    # 繪圖和統計處理（此處省略部分代碼，保持您的繪圖程式碼不變）
