...
if not st.session_state.authenticated:
    ...
    # 自動載入測試檔案的按鈕
    if st.button("使用內嵌測試檔案") and not st.session_state.file_loaded:
    ...
    with st.expander("請上傳Excel文件:【權利金】歷年總表紀錄 "):
        ...
        if uploaded_file and not st.session_state.file_loaded:
            st.session_state.pre_data = load_uploaded_file(uploaded_file)
    ...
    if st.session_state.pre_data is not None:
        ...
        with st.expander("請選擇條件"):
            ...
            if view_option == "歷年加總":
                ...
                recent_3years_rank = recent_3years_data.groupby(by=['合約簡編'])[['電子書內容收益', '權利金']].sum().round(0).reset_index().sort_values(by='電子書內容收益', ascending=False)
